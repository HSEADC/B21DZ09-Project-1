...
;// CONCATENATED MODULE: ./src/javascript/quiz-init.js
...
var content;
var results = document.querySelector('.S_QuizResults');
var buttonAgain = results.querySelector('.M_ButtonSurvey');
var cover = document.querySelector('.S_Cover');
var quizButton = document.querySelector('.S_Cover .M_ButtonSurvey');
var quiz = document.querySelector('.S_QuizBlock');
var tags = quiz.querySelectorAll('.M_SurveyTag');
var buttonNext = quiz.querySelector('.M_ButtonSurvey');
var buttonBack = quiz.querySelector('.A_ButtonBack');
var buttonClose = quiz.querySelector('.A_Close');
var counter = quiz.querySelector('.A_Counter');
var stage = quiz.querySelectorAll('.A_CircleStage');
var progressLine = quiz.querySelector('.A_ProgressLine');
var n = 1;
var allActive = [];
var activePlacesFin = [];
var activeMoodsFin = [];
var activeCharacterFin = [];
...
function createContentCard(contentItemData) {
  ...
  var contentItemCoverQ = document.createElement('div');
  contentItemCoverQ.classList.add('Q_Image');
  contentItemCover.appendChild(contentItemCoverQ);
  var contentItemCoverImg = document.createElement('img');
  ...
  contentItemData.mood.forEach(function (mood) {
...
function renderQuizContent() {
  ...
  var contentItemsContainer = results.querySelector('.C_Styles');
  contentItemsContainer.innerHTML = '';
  var contentItemIds = [];
  content.forEach(function (contentItem) {
    var place = contentItem.place,
        mood = contentItem.mood,
        character = contentItem.character;

    if (place.some(function (r) {
      return activePlacesFin.includes(r);
    }) && mood.some(function (r) {
      return activeMoodsFin.includes(r);
    }) && character.some(function (r) {
      return activeCharacterFin.includes(r);
    }) && contentItemIds.length < 3 || place.some(function (r) {
      return activePlacesFin.includes(r);
    }) && mood.some(function (r) {
      return activeMoodsFin.includes(r);
    }) && contentItemIds.length < 3 || mood.some(function (r) {
      return activeMoodsFin.includes(r);
    }) && character.some(function (r) {
      return activeCharacterFin.includes(r);
...
document.addEventListener('DOMContentLoaded', function () {
  getQuizCards().then(function (data) {
    content = data;
    quizInit();
  });
});
